...
4. **Measured boundary** — the two curves above are a model, so **Measure this column** goes and checks them. For the current signature count it runs three complete end-to-end attacks (fresh key, fresh signatures, real LLL, byte-for-byte validation) at each of six leak sizes straddling the information floor, prints the observed recovery rate per rung, and states where the measurement and the drawn curve disagree. Measured columns accumulate as dots over the modelled curves. Where it cannot honestly measure — a leak mode with no bits axis, a signature count too slow to sweep in a tab, or a column whose floor is past the 32-bit control — it declines and gives the reason instead of drawing a boundary it did not run.
